...
function Peer(options) {
  ...
  console.log("################### Peer options.network : " + JSON.stringify(options.network) );
  
  this.network = Networks.get(options.network) || Networks.defaultNetwork;

  console.log("################### Peer.network : " + JSON.stringify(this.network) );
  ...
  this.on('verack', function() {
	console.log("################### P2P peer on event : verack" );
    self.status = Peer.STATUS.READY;
    self.emit('ready');
  });

  this.on('version', function(message) {
	console.log("################### P2P peer on event : version ,bestHeight is : "  + message.startHeight );
  ...
  this.on('ping', function(message) {
	console.log("################### P2P peer on event : ping" );
...
Peer.prototype.sendMessage = function(message) {
  console.log("################### P2P peer send msg : " + JSON.stringify(message) );
  //console.log("################### P2P peer send msg : " + message.toBuffer().toString() );
...
Peer.prototype._readMessage = function() {
  ...
  if (message) {
	//console.log("$$$$$$$$$$$$$$$$$$ message.command is : " + message.command );
