...
HeadersMessage.prototype.setPayload = function(payload) {
  ...
  console.log("################# HeadersMessage read count: " + count);
  this.headers = [];
  for (var i = 0; i < count; i++) {
    var header = this.BlockHeader.fromBufferReader(parser);
    this.headers.push(header);
	console.log("################# HeadersMessage read header: " +  JSON.stringify(header) );
	//去掉txn_count
    var txn_count = parser.readUInt8();
	var txn_count2 = parser.readUInt16LE();
	//var txn_count = parser.readUInt32LE();
