...
function Pool(options) {
  ...
  //console.log("################### Pool options.network : " + JSON.stringify(options.network) );
  ...
  //console.log("################### Pool.addrs : " + JSON.stringify(this._addrs) );
...
Pool.prototype._fillConnections = function _fillConnections() {
  //console.log("################# Pool _fillConnections this._addrs: " + JSON.stringify(this._addrs));
...
Pool.prototype._connectPeer = function _connectPeer(addr) {
  var self = this;
  //console.log("Pool _connectPeer addr : " + JSON.stringify(addr) );
...
Pool.prototype._addPeerEventHandlers = function(peer, addr) {
  ...
  peer.on('ready', function peerReady() {
	console.log("################### P2P peer on event : ready" );
    self.emit('peerready', peer, addr);
  });
  Pool.PeerEvents.forEach(function addPeerEvents(event) {
	//console.log("################ reg on event : " + JSON.stringify(event) );
    peer.on(event, function peerEvent(message) {
	  //console.log("################ peer on event : " + JSON.stringify(event) );
...
Pool.prototype._addAddr = function _addAddr(addr) {
  ...
  console.log("################ _addAddr addr : " + JSON.stringify(addr) + "this.network.port: " + this.network.port );
...
Pool.prototype._addAddrsFromSeeds = function _addAddrsFromSeeds() {
  console.log("################### _addAddrsFromSeeds" );
  var self = this;
